...
export function initControls({ settings, onChange = () => {} }) {
    ...
    settings.background.colors.forEach((color, index) => {
      ...
      row.querySelectorAll('.move-color').forEach((button) =>
        button.addEventListener('click', () => {
          ...
          [settings.background.colors[index], settings.background.colors[next]] = [
            settings.background.colors[next],
  ...
  function renderEffectSettings() {
    ...
    effectSettings.innerHTML = isSolid
      ? '<p class="hint">Solid uses the first background color and does not run on division.</p>'
      : `<div class="control-label">${effect[0].toUpperCase() + effect.slice(1)} controls</div>`;
    (BACKGROUND_EFFECT_CONTROL_SPECS[effect] || []).forEach(([key, label, min, max, suffix]) => {
      ...
      input.addEventListener('input', () => {
        ...
        if (effect === 'sparkle' && key === 'minSize')
          settings.background.effectSettings.sparkle.maxSize = Math.max(
            settings.background.effectSettings.sparkle.maxSize,
            next,
          );
        if (effect === 'sparkle' && key === 'maxSize')
          settings.background.effectSettings.sparkle.minSize = Math.min(
            settings.background.effectSettings.sparkle.minSize,
            next,
          );
  ...
  function syncControls() {
    ...
    backgroundDivisions.forEach((button) =>
      button.setAttribute(
        'aria-pressed',
  ...
  COLOR_PRESETS.forEach(([name, palette], index) => {
    ...
    button.addEventListener('click', () => {
      settings.background.colors = [...palette];
      renderColors();
      onChange(settings);
    });
