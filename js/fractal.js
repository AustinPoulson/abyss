(function (global) {
  ...
  function normalize(config) {
    ...
    return {
      ...
      colors: Array.isArray(source.colors) && source.colors.length ? source.colors.slice() : DEFAULT_COLORS.slice(),
  ...
  function drawInk(context, radius, halfAngle, settings, random, step) {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      context.bezierCurveTo(
        end * 0.28, Math.sin(bend * 3) * radius * 0.18,
        end * 0.68, (random() - 0.5) * radius * halfAngle * 0.8,
        end, Math.tan((random() - 0.5) * halfAngle * 1.7) * end
      );
      context.stroke();
    }
  }
  ...
  function drawLiquid(context, radius, halfAngle, settings, random, step) {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      context.bezierCurveTo(length * 0.15, width * (0.8 + random()), -length * 0.18, width * (0.7 + random()), -length * 0.5, 0);
      context.fill();
      context.restore();
    }
  }
  ...
  function rotationAt(rotation, seconds, beatPhase) {
    ...
    if (rotation === "counterclockwise" || rotation === "anticlockwise" || rotation === "ccw") return seconds * -0.18;
    if (rotation === "alternate" || rotation === "alternating") return Math.sin(seconds * 0.55) * 0.8;
    if (typeof rotation === "string" && Number.isFinite(Number(rotation))) return seconds * Number(rotation) * Math.PI / 180;
    return beatPhase * 0;
  }
  ...
  function start(canvas, config) {
    if (!canvas || typeof canvas.getContext !== "function") throw new TypeError("AbyssFractal.start requires a canvas element.");
    ...
    const reducedQuery = typeof global.matchMedia === "function" ? global.matchMedia("(prefers-reduced-motion: reduce)") : null;
    ...
    function resize() {
      ...
      sourceRadius = Math.hypot(Math.max(centerX, width - centerX), Math.max(centerY, height - centerY)) * 1.12;
      const logicalDiameter = sourceRadius * 2;
      const sourcePixels = Math.max(256, Math.min(1400, Math.ceil(logicalDiameter * Math.min(pixelRatio, 1.5))));
    ...
    function renderSource(step) {
      ...
      if (hasCurrentSource && previousSourceContext) {
        ...
        previousSourceContext.clearRect(0, 0, previousSourceCanvas.width, previousSourceCanvas.height);
      ...
      sourceContext.clip();
      const painter = { ink: drawInk, shards: drawShards, spray: drawSpray, liquid: drawLiquid }[settings.style];
    ...
    function draw(now, force) {
      ...
      const crossfadeProgress = reducedMotion || !hasPreviousSource || crossfadeSpan === 0
        ? 1
    ...
    function stop() {
      ...
      if (reducedQuery) {
        if (typeof reducedQuery.removeEventListener === "function") reducedQuery.removeEventListener("change", onMotionChange);
        else if (typeof reducedQuery.removeListener === "function") reducedQuery.removeListener(onMotionChange);
      }
      context.clearRect(0, 0, width, height);
    }
    ...
    if (reducedQuery) {
      if (typeof reducedQuery.addEventListener === "function") reducedQuery.addEventListener("change", onMotionChange);
      else if (typeof reducedQuery.addListener === "function") reducedQuery.addListener(onMotionChange);
    }
