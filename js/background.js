...
export function startBackground(settings) {
    ...
    function drawEffects() {
      ...
      const fadeDuration = (60000 / settings.tempo) * BEAT_DIVISIONS[settings.background.division] * (sparkleSettings.fadeIn / 100);
    ...
    if (settings.background.effect === "slime" && !reduced) {
      ...
      function slimeFrame(now) {
        const colors = settings.background.colors; const interval = (60000 / settings.tempo) * BEAT_DIVISIONS[settings.background.division]; const steps = interval ? Math.floor((now - lastSlime) / interval) : 1;
