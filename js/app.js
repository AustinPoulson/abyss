import {
  BACKGROUND_EFFECT_CONTROL_SPECS,
  BEAT_DIVISIONS,
  cloneSettings,
  COLOR_PRESETS,
  DEFAULTS,
...
(function () {
  ...
  async function loadSettings() {
    try {
      ...
      try {
        return normalizeSettings(JSON.parse(localStorage.getItem('abyss-settings')));
      } catch (error) {
  ...
  function startStrobe() {
    ...
    function drawEffects() {
      ...
      const fadeDuration =
        ...
        beats[settings.background.division] *
  ...
  function startFractalLayer() {
    ...
    window.AbyssFractal.start(
      canvas,
      Object.assign({}, settings.fractal, {
  ...
  function initControls() {
    ...
    if (!presetList.id) {
      ...
      colorList?.parentNode.insertBefore(presetList, colorList);
    }
    ...
    const effectSettingsHost = document.createElement('div');
    effectSettingsHost.className = 'effect-settings';
    divisionHint?.after(effectSettingsHost);
    ...
    function renderEffectSettings() {
      ...
      effectSettingsHost.innerHTML = isSolid
        ? '<p class="hint">Solid uses the first background color and does not run on division.</p>'
        : `<div class="control-label">${effect.charAt(0).toUpperCase() + effect.slice(1)} controls</div>`;
      (effectControlSpecs[effect] || []).forEach(([key, label, min, max, suffix]) => {
        ...
        input.addEventListener('input', () => {
          settings.background.effectSettings[effect][key] = Number(input.value);
          if (
            effect === 'sparkle' &&
            key === 'minSize' &&
            settings.background.effectSettings.sparkle.maxSize < Number(input.value)
          )
            settings.background.effectSettings.sparkle.maxSize = Number(input.value);
          if (
            effect === 'sparkle' &&
            key === 'maxSize' &&
            settings.background.effectSettings.sparkle.minSize > Number(input.value)
          )
            settings.background.effectSettings.sparkle.minSize = Number(input.value);
    ...
    function syncFractalControls() {
      ...
      document
        .querySelectorAll('[data-fractal-division]')
        .forEach((button) =>
          button.setAttribute(
            'aria-pressed',
            String(button.dataset.fractalDivision === settings.fractal.division),
          ),
        );
    ...
    const renderColorPreview = () => {
      ...
      colorPreview.setAttribute(
        'aria-label',
        `Current background colors: ${settings.background.colors.join(', ')}`,
      );
    ...
    const renderColors = () => {
      ...
      settings.background.colors.forEach((color, index) => {
        ...
        row.querySelectorAll('.move-color').forEach((button) =>
          button.addEventListener('click', () => {
            const nextIndex = button.dataset.direction === 'up' ? index - 1 : index + 1;
            if (nextIndex < 0 || nextIndex >= settings.background.colors.length) return;
            [settings.background.colors[index], settings.background.colors[nextIndex]] = [
              settings.background.colors[nextIndex],
              settings.background.colors[index],
            ];
            renderColors();
            saveSettings();
          }),
    ...
    presets.forEach(([name, colors], index) => {
      ...
      button.addEventListener('click', () => {
        settings.background.colors = [...colors];
        renderColors();
        saveSettings();
      });
    ...
    document.getElementById('add-color')?.addEventListener('click', () => {
      settings.background.colors.push('#07090c');
      renderColors();
      saveSettings();
    });
    if (tempo && tempoValue) {
      ...
      tempo.addEventListener('input', () => {
        settings.tempo = Number(tempo.value);
        tempoValue.textContent = `${tempo.value} BPM`;
        saveSettings();
      });
    }
    divisions.forEach((button) => {
      button.setAttribute(
        'aria-pressed',
        String(button.dataset.division === settings.background.division),
      );
      button.addEventListener('click', () => {
        settings.background.division = button.dataset.division;
        divisions.forEach((item) => item.setAttribute('aria-pressed', String(item === button)));
        saveSettings();
      });
    });
    effects.forEach((button) => {
      button.setAttribute(
        'aria-pressed',
        String(button.dataset.effect === settings.background.effect),
      );
      button.addEventListener('click', () => {
        settings.background.effect = button.dataset.effect;
        effects.forEach((item) => item.setAttribute('aria-pressed', String(item === button)));
    ...
    document.querySelectorAll('[data-fractal-choice]').forEach((button) =>
      button.addEventListener('click', () => {
        const key = button.dataset.fractalChoice;
        settings.fractal[key] =
          key === 'enabled' ? button.dataset.value === 'true' : button.dataset.value;
        syncFractalControls();
        saveSettings();
      }),
    );
    document.querySelectorAll('[data-fractal-division]').forEach((button) =>
      button.addEventListener('click', () => {
        settings.fractal.division = button.dataset.fractalDivision;
        syncFractalControls();
        saveSettings();
      }),
    );
    document.querySelectorAll('[data-fractal-range]').forEach((input) =>
      input.addEventListener('input', () => {
        settings.fractal[input.dataset.fractalRange] = Number(input.value);
        input.nextElementSibling.textContent = `${input.value}${input.dataset.suffix || ''}`;
        saveSettings();
      }),
    );
